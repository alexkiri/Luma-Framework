...
#if ENABLE_LUMA
  r2.xyz = pow(abs(r1.xyz), 1.0 / 2.2) * sign(r1.xyz);
#else
  r2.xyz = pow(abs(r1.xyz), 1.0 / 2.2);
#endif
  r2.xyz -= 0.5;
  // Calculate some kind of luminance for film grain
  r2.xyz = r2.xyz * cb1[0].z + 0.5;
...
#endif
  r0.xyz = lerp(r1.xyz, r0.xyz, cb1[0].x); // Filter intensity
