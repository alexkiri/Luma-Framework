...
{
  //TODOFT: this decomp is broken, the lighting at the edges looks different and I can't figure out why (decompiling with DXVK provided the same result, maybe NV replaces the shader through drivers?).
  //The other character ones I decompiled (not all of them) seem fine instead, but that's not 100% sure.
  //I tried to change all compiler flags and the output is still different. The only thing left to do is comparing the asm.
