...
Texture2D<float4> PostAA_DepthTex : register(t2); // LUMA FT: unused. This is the linearized+normalized depth (0 being the zero (a zero distance from the camera origin, not the near), and 1 being the far). This might be full (relative) resolution or scaled (unknown yet)
...
// LUMA FT: this can only be used with "PostAA_DepthTex" and not "PostAA_DeviceDepthTex"
