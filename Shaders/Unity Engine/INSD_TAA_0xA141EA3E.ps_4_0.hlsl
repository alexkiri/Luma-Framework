...
Texture2D<float4> t4 : register(t4); // Noise
  ...
  out float4 o1 : SV_Target1)
{
  ...
  r0.yz = -cb0[15].xy * cb0[11].xy + v1.xy;
  r3.xyzw = SafeSceneSample(r0.yz).xyzw;
  r1.zw = v1.xy - r1.xy;
  ...
  r0.xw = v1.xy * cb0[11].zw + cb0[13].xy;
  r0.xw = cb0[12].xy * r0.xw;
  float4 noise = t4.SampleLevel(s3_s, r0.xw, 0).xyzw; // Defaults to white?

  r0.xw = cb0[20].zz * r1.xy;
  r1.xy = r0.xw / cb0[11].xy;
  r1.x = length(r1.xy); // MVs length
  r1.x -= 2.0;
  r1.x = max(0, r1.x);
  r1.x = min(13, r1.x);
  r1.x = -r1.x * 0.0769230798 + 1;
  r1.y = -0.5 + noise.x;
  ...
  while (true) {
    ...
    sceneColor += SafeSceneSample(uv).xyzw;
    i++;
  }
  sceneColor /= 7.0;
#if 0 // Test: ~passthrough
  o0.xyzw = sceneColor * 1;
  o1.xyzw = sceneColor * 1;
  return;
#endif

  r0 = lerp(sceneColor, r2, r1.x); // Raw color blend? Basically the TAA percentage?
  noise = (noise * 2.0) - 1.0; // From 0|1 to -1|1
  noise /= 255.0; // Reduce noise (not sure why 255 was picked, the texture wasn't 8bit UINT)
#if !ENABLE_FILM_GRAIN // This isn't film grain but it's not far from it either
  noise = 0.0;
#endif
  // Output two similar colors, index 0 is the history (only read again the next frame, by this very shader), index 1 is the actual output that continues to be used for this frame's post processing
