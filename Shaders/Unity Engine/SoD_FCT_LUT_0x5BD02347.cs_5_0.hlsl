...
void main(uint3 vThreadID : SV_DispatchThreadID)
{
  ...
  if (r0.w != 0) {
    ...
    r1.xyz = max(float3(0,0,0), r1.xyz); //TODO: remove and abs etc
    r1.xyz = log2(r1.xyz);
    ...
    r1.xyz = float3(0.179999992,0.179999992,0.179999992) * r1.xyz;
#if 0 // LUMA: fixed negative values support (this seems to break rendering...)
    r1.xyz = pow(abs(r1.xyz), 1.0 / 2.2) * sign(r1.xyz);
#else
    r1.xyz = pow(max(r1.xyz, 0.0), 1.0 / 2.2);
#endif
    r2.xyz = min(float3(1,1,1), r1.xyz); //TODO: remove min? And test above for follow up nans. Also, is it right that this runs in Rec.709 while the other LUT uses AP1 luminance coeffs?
...
#endif
  _OutputTexture[vThreadID.xyz] = float4(r0.xyz, 1.0);
}