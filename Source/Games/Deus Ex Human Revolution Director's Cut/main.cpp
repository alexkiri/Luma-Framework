...
public:
   ...
   std::unique_ptr<std::byte[]> ModifyShaderByteCode(const std::byte* code, size_t size, reshade::api::pipeline_subobject_type type) override
   {
      ...
      std::vector<std::byte> pattern_sat_fix;
      if (!matches_mad_sat.empty())
      {
         matches_sat = matches_mad_sat;
         pattern_sat_fix = pattern_mad;
      }
      ...
      else if (false) // TODO: fix
      {
         ...
         if (matches_mad.size() >= 2 && !matches_mul_sat.empty())
         {
            size_t last_mads_offset = matches_mad[matches_mad.size() - 2] - matches_mad[matches_mad.size() - 1];
            size_t last_mad_to_mul_sat_offset = matches_mad[matches_mad.size() - 1] - matches_mul_sat[matches_mul_sat.size() - 1];
            // 5 DWORD
            ...
            if (last_mads_offset == 20 && last_mad_to_mul_sat_offset == 16)
            {
               matches_sat = matches_mul_sat;
               pattern_sat_fix = pattern_mul;
            }
         }
      }
      ...
      if (!matches_sat.empty())
      {
         ...
         {
            std::vector<std::byte*> matches_bt_601_luminance = System::ScanMemoryForPattern(reinterpret_cast<const std::byte*>(matches_sat.back()), size - size_offset, pattern_bt_601_luminance);
            if (!matches_bt_601_luminance.empty())
            {
               ...
               std::memcpy(new_code.get() + offset, pattern_sat_fix.data(), pattern_sat_fix.size());
   ...
   bool OnDrawCustom(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, reshade::api::shader_stage stages, const ShaderHashesList<OneShaderPerPipeline>& original_shader_hashes, bool is_custom_pass, bool& updated_cbuffers) override
   {
      ...
      if (!game_device_data.has_drawn_custom_gold_filter)
      {
         ...
         else if (original_shader_hashes.Contains(pixel_shader_hashes_BloomComposition))
         {
            ...
            SetLumaConstantBuffers(native_device_context, cmd_list_data, device_data, stages, LumaConstantBufferType::LumaSettings);
            SetLumaConstantBuffers(native_device_context, cmd_list_data, device_data, stages, LumaConstantBufferType::LumaData, 0, 0, bloom_intensity);
            updated_cbuffers = true;
         }
         // Materials rendering
         else if (!device_data.has_drawn_main_post_processing) // TODO: exclude UI from these... Also only do this after some crucial normal map etc phase finished
         {
            ...
            if (r2 && r2 == r1)
            {
               ...
               if (blend_state)
               {
                  ...
                  if (!game_device_data.has_drawn_opaque_geometry && !IsRTRGBBlendDisabled(blend_desc.RenderTarget[0]) && test_index != 18)
                  {
                     ...
                     if (trace_running) // TODO: why doesn't this one end up visible in the list?!
                     {
                        ...
                        || blend_desc.RenderTarget[0].DestBlendAlpha == D3D11_BLEND_BLEND_FACTOR))
                  {
                     ...
                     if (blend_desc.RenderTarget[0].DestBlend == D3D11_BLEND_BLEND_FACTOR)
                     {
                        ...
                        if (test_index == 17) // TODO: try and output an alpha from the pixel shader and use that to darken the background? It works but doesn't seem to help
                        {
   ...
   void OnPresent(ID3D11Device* native_device, DeviceData& device_data) override
   {
      ...
      if (device_data.has_drawn_main_post_processing)
      {
         ...
         has_supported_aa_count = std::clamp(has_supported_aa_count, 0, 60); // TODO: find best value
      }
   ...
   void DrawImGuiSettings(DeviceData& device_data) override
   {
      ...
      if (ImGui::SliderFloat("Bloom Intensity", &bloom_intensity, 0.f, 1.f))
      {
